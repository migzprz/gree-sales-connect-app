		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getParts/`,
				);
		...
		if (form.checkValidity()) {
			try {
				...
				const postResponse = await axios.post(
					`${process.env.REACT_APP_ROOT_URL}/api/postWarrantyParts/${id}`,
					{ data: postData },
				);
								...
								<Row key={index} className="mt-1">
									...
									<Col lg="1">
										{React.createElement(FaTrash, {
											...
											style: { cursor: "pointer", marginRight: "5px", marginTop: "10px" },
										})}
									</Col>
								</Row>
							))}
