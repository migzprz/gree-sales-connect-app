	...
	const handleSubmit = async (event) => {
		...
		try {
			const response = await axios.patch(
				`${process.env.REACT_APP_ROOT_URL}/api/changeOcularState/${id}/0`,
			);
			...
			<Modal show={showModal} onHide={handleCloseModal} size="lg">
				...
				<Form noValidate validated={validated} onSubmit={handleSubmit}>
					<Modal.Body style={{ color: "#8c1919", backgroundColor: "#E5EDF4" }}>
						<Row>
							<Col>
								Are you sure you want to <strong> cancel </strong> the ocular with the
								following details?
							</Col>
						</Row>
						<Row className="mt-4">
						...
						<Row>
							...
							<Col>{new Date(record.ocular_date).toLocaleDateString()}</Col>
						</Row>
						...
						<Row>
							...
							<Col>{new Date(record.ocular_date).toLocaleTimeString()}</Col>
						</Row>
						...
						<Row>
							...
							<Col>{record.technician_name}</Col>
						</Row>
					</Modal.Body>
