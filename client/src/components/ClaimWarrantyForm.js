...
const ClaimWarrantyForm = () => {
	...
	useEffect(() => {
		...
		if (formData.inspection_dateonly && formData.inspection_time) {
			...
			const newInputDateTime =
				formData.inspection_dateonly + "T" + formData.inspection_time;
	...
	useEffect(() => {
		...
		if (formData.service_dateonly && formData.service_time) {
			// Both date and time are present, update inputDateTime
			const newInputDateTime =
				formData.service_dateonly + "T" + formData.service_time;
		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getWarrantySearchDetails/${id}`,
				);
				const response2 = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getDeductableUnits/`,
				);
		...
		if (form.checkValidity()) {
			try {
				const postReponse = await axios.post(
					`${process.env.REACT_APP_ROOT_URL}/api/postWarranty`,
					{ data },
				);
						...
						<Col>
							<Row>
								<Col>
									...
									<strong>
										{" "}
										{warrantySearchData.length > 0 &&
											warrantySearchData[0] &&
											warrantySearchData[0].sales_id}
									</strong>
								</Col>
							</Row>
							<Row>
								<Col>
									...
									<strong>
										{warrantySearchData.length > 0 &&
											warrantySearchData[0] &&
											warrantySearchData[0].client_name}
									</strong>
								</Col>
							</Row>
							<Row>
								<Col>
									...
									<strong>
										...
										{warrantySearchData.length > 0 &&
											warrantySearchData[0] &&
											warrantySearchData[0].company_name}
									</strong>
								</Col>
							</Row>
							<Row>
								<Col>
									...
									<strong>
										...
										{warrantySearchData.length > 0 &&
											warrantySearchData[0] &&
											warrantySearchData[0].client_number}
									</strong>
								</Col>
							</Row>
							<Row>
						...
						<Row className="mt-1">
							<Col lg="3">
								<Form.Group controlId="description">
									...
									<Form.Control
										type="date"
										name="inspection_dateonly"
										value={formData.inspection_dateonly}
										onChange={(e) => handleChangeDate(e, "inspection_dateonly")}
										required
									/>
							...
							<Col lg="3">
								<Form.Group controlId="description">
									...
									<Form.Control
										...
										onChange={(e) => handleChangeTime(e, "inspection_time")}
										required
									/>
									<Form.Control.Feedback type="invalid">
							...
							<Col lg="3">
								<Form.Group controlId="type">
									...
									<Form.Control
										as="select"
										name="inspection_technician_id"
										disabled={!formData.inspection_dateonly || !formData.inspection_time}
										value={formData.inspection_technician_id}
										onChange={(e) =>
											handleChangeTechnician(e, "inspection_technician_id")
										}
										required
									>
						...
						<Row className="mt-2">
							<Col lg="3">
								<Form.Group controlId="description">
									...
									<Form.Control
										type="date"
										name="service_dateonly"
										value={formData.service_dateonly}
										onChange={(e) => handleChangeDate(e, "service_dateonly")}
										required
									/>
									<Form.Control.Feedback type="invalid">
							...
							<Col lg="3">
								<Form.Group controlId="description">
									...
									<Form.Control
										...
										onChange={(e) => handleChangeTime(e, "service_time")}
										required
									/>
									<Form.Control.Feedback type="invalid">
							...
							<Col lg="3">
								<Form.Group controlId="type">
									...
									<Form.Control
										as="select"
										name="service_technician_id"
										disabled={!formData.service_dateonly || !formData.service_time}
										value={formData.service_technician_id}
										onChange={(e) => handleChangeTechnician(e, "service_technician_id")}
										required
									>
										<option value=""> Select </option>
														...
														<tr style={{ borderRadius: "20px", padding: "10px" }}>
															<td style={{ color: "#014c91" }}>
																<Form.Check
																	...
																	required={formData.claimed_units.every(
																		(unit) => unit.for_claiming === 0,
																	)}
																/>
															</td>
															<td style={{ color: "#014c91" }}>
																...
																{item.for_claiming ? (
																	<Form.Group controlId="role">
																		<Form.Control
																			...
																			name="issue"
																			value={item.issue}
																			onChange={(e) => handleChange(e, index, "issue")}
																			required
																		>
																			<option value=""> Select </option>
																			<option value="Overheating Unit"> Overheating Unit</option>
																			<option value="Inadequate Cooling">
																				{" "}
																				Inadequate Cooling{" "}
																			</option>
