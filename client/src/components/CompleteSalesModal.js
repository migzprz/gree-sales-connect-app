...
const CompleteSalesModal = ({ id }) => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (form.checkValidity()) {
            try {
                const patchResponse = await axios.patch(`http://localhost:4000/api/changeSaleState/${id}/1`);
                ...
                <Form noValidate validated={validated} onSubmit={handleSubmit}>
                    ...
                    <Modal.Footer style={{backgroundColor: "#E5EDF4"}}>
                            ...
                            <button className="btn" style={{color: "white", backgroundColor:  "#014c91"}} onClick={handleSubmit}>
                                {React.createElement(FaCheck, { size: 18, style: { marginRight: '5px' } })} Complete Sale
                            </button>
...
export default CompleteSalesModal;
