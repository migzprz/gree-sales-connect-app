...
const CompleteServiceModal = ({ type, id, refId }) => {
	...
	const handleSubmit = async () => {
		if (type === "delivery" || type === "pickup") {
			try {
				const res = await axios.patch(
					`${process.env.REACT_APP_ROOT_URL}/api/completeDelivery/${id}`,
				);
				console.log(res);
			} catch (error) {
				console.error(error);
		...
		if (type === "installation") {
			try {
				const res = await axios.patch(
					`${process.env.REACT_APP_ROOT_URL}/api/completeInstallation/${id}`,
				);
				console.log(res);
			} catch (error) {
				console.error(error);
		...
		if (type === "service") {
			try {
				const res = await axios.patch(
					`${process.env.REACT_APP_ROOT_URL}/api/completeService/${id}`,
				);
				console.log(res);
			} catch (error) {
				console.error(error);
		...
		<div>
			...
			<Modal show={showModal} onHide={handleCloseModal}>
				...
				<Modal.Body style={{ color: "#014c91", backgroundColor: "#E5EDF4" }}>
					{type === "delivery" ? (
						<Row>
							<Col>Complete delivery for Sale Ref. #{refId}</Col>
						</Row>
					) : type === "installation" ? (
						<Row>
							<Col>Complete installation for Sale Ref. #{refId}</Col>
						</Row>
					) : type === "pickup" ? (
						<Row>
							<Col>Complete pick up for Sale Ref. #{refId}</Col>
						</Row>
					) : (
						<Row>
							<Col>Complete service for Sale Ref. #{refId}</Col>
						</Row>
					)}
				</Modal.Body>
