...
const CompleteWarrantyModal = ({ id }) => {
		...
		if (form.checkValidity()) {
			try {
				const patchResponse = await axios.patch(
					`${process.env.REACT_APP_ROOT_URL}/api/changeWarrantyState/${id}/1`,
				);
			...
			<Modal show={showModal} onHide={handleCloseModal}>
				...
				<Form noValidate validated={validated} onSubmit={handleSubmit}>
					<Modal.Body style={{ color: "#014c91", backgroundColor: "#E5EDF4" }}>
						<Row>
							<Col>
								Complete warranty for <strong>Claim #{id}? </strong> <br />
								No more changes can be made to this warranty claim.
							</Col>
						</Row>
					</Modal.Body>
...
export default CompleteWarrantyModal;
