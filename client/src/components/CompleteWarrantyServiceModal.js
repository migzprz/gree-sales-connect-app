			...
			if (data.for_service && type === "inspection") {
				try {
					const patchResponse = await axios.patch(
						`${process.env.REACT_APP_ROOT_URL}/api/changeWarrantyInspectionState/${service_id}/1/${data.note}`,
						data,
					);
					console.log(patchResponse);
					const postReponse = await axios.post(
						`${process.env.REACT_APP_ROOT_URL}/api/postWarrantyInspection`,
						{ data },
					);
				...
				try {
					const patchResponse = await axios.patch(
						`${process.env.REACT_APP_ROOT_URL}/api/changeWarrantyServiceState/${service_id}/1/${data.note}`,
					);
					console.log(patchResponse);
					const postReponse = await axios.post(
						`${process.env.REACT_APP_ROOT_URL}/api/postWarrantyService`,
						{ data },
					);
					console.log(postReponse);
	...
	return (
		<div>
			...
			>
				...
				{type === "inspection" ? "Complete Inspection" : "Complete Service"}
			</button>
			...
			<Modal show={showModal} onHide={handleCloseModal} size="lg">
				<Modal.Header
					style={{
						color: "white",
						backgroundColor: record.is_active ? "#8c1919" : "#014c91",
					}}
				>
					<Modal.Title>
						{type === "inspection" ? "Complete Inspection" : "Complete Service"}
