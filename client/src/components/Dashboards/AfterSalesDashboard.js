...
const AfterSalesDashboard = () => {
		...
		const fetchWarrantyData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getWarrantyStatistics/`,
				);
				const response2 = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getResolvedWarrantyStatistics/`,
				);
				let newData = [];
		...
		const fetchUnitData = async () => {
			try {
				...
				if (monthFilter === 0) {
					// If monthFilter is empty, generate data for all months
					const response = await axios.get(
						`${process.env.REACT_APP_ROOT_URL}/api/getYearlyClaimedUnitStatistics/`,
					);
					...
					const response = await axios.get(
						`${process.env.REACT_APP_ROOT_URL}/api/getClaimedUnitStatistics/`,
					);
					newData = response.data
						.filter(
							(item) => item.year === yearFilter && item.monthNum === monthFilter,
						)
						.map((item) => ({ name: item.issue, unit: item.qty }));
					total = newData.reduce((acc, item) => acc + item.unit, 0);
				}
	...
	const [ocularLinesVisibility, setOcularLinesVisibility] = useState({
		ocular: true,
	});
	const [quotationLinesVisibility, setquotationLinesVisibility] = useState({
		quotation: true,
	});
	const [salesLinesVisibility, setsalesLinesVisibility] = useState({
		sales: true,
	});
									...
									<ResponsiveContainer width="100%" height={300}>
										...
										<LineChart data={warrantyData}>
											...
											<Line
												type="monotone"
												dataKey="warranty"
												name="Warranties Claimed"
												stroke="#1427E2"
											/>
											<Line
												type="monotone"
												dataKey="resolved"
												name="Warranties Resolved"
												stroke="#FFA500"
											/>
										</LineChart>
									</ResponsiveContainer>
									...
									<ResponsiveContainer width="100%" height={300}>
										...
										<BarChart data={unitData}>
											...
											<Bar
												...
												name="Claimed Units"
												fill="#893101"
												activeBar={<Rectangle fill="#D67229" stroke="blue" />}
											/>
										</BarChart>
									</ResponsiveContainer>
								</CardBody>
