...
const SalesDashboard = () => {
	...
	useEffect(() => {
		const fetchOcularData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getOcularStatistics/`,
				);
				let newData = [];
		...
		const fetchQuotationData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getQuotationStatistics/`,
				);
				const response2 = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getSalesStatistics/`,
				);
				let newData = [];
				let total = 0; // Initialize total
		...
		const fetchRevenueData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getRevenueStatistics/`,
				);
		...
		const fetchData = async () => {
			try {
				...
				const [revenueResponse, ocularResponse, quotationResponse] =
					await Promise.all([
						fetchRevenueData(),
						fetchOcularData(),
						fetchQuotationData(),
								...
								<CardBody className="mb-5">
									<ResponsiveContainer width="100%" height={300}>
										...
										<LineChart data={ocularData}>
											...
											<Tooltip />
											<Legend />
											{Object.keys(ocularLinesVisibility).map(
												(key, index) =>
