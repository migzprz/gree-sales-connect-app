		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getClient/${id}`,
				);
	...
	const handleSubmit = async (event) => {
		...
		if (form.checkValidity()) {
			if (editCompanyData.company_name !== "" || editCompanyData.tin !== "") {
				try {
					const postReponse2 = await axios.patch(
						`${process.env.REACT_APP_ROOT_URL}/api/updateCompany/${record.company_id}`,
						editCompanyData,
					);
			...
			if (
				...
				try {
					const postReponse = await axios.patch(
						`${process.env.REACT_APP_ROOT_URL}/api/updateContactPerson/${record.contact_person_id}`,
						editData,
					);
								...
								<Col lg="4">
									<Form.Group controlId="tin">
										...
										<Form.Control
											type="text"
											pattern="[0-9]{9,12}"
											value={editCompanyData.tin}
											placeholder={record.tin}
											name="tin"
											onChange={handleCompanyChange}
										/>
										<Form.Control.Feedback type="invalid">
