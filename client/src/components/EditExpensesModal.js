...
const EditExpensesModal = ({ id }) => {
		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getExpenseDetails/${id}`,
				);
				setRecord(response.data[0]);
				const response2 = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getOperatingExpenses/${id}`,
				);
				setOpExpenseData(response2.data);
				const response3 = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getNonOperatingExpenses/${id}`,
				);
	...
	useEffect(() => {
		setExpenseList((prevExpenseList) => {
			...
			const dateCreated = record.date_created
				? new Date(record.date_created).toISOString().slice(0, 19).replace("T", " ")
				: null;
			...
			const newExpenseType2 = {
				type: "Non-Operating Exp.",
				is_operating: 0,
				date: dateCreated,
				expense: [],
			};
			...
			for (let opexpense of opExpenseData) {
				let formattedDate = new Date(opexpense.expense_date)
			...
			for (let nonopexpense of nonopExpenseData) {
				...
				let formattedDate = new Date(nonopexpense.expense_date)
					.toLocaleDateString("en-GB", {
						year: "numeric",
						month: "2-digit",
						day: "2-digit",
		...
		if (form.checkValidity()) {
			try {
				const deleteReponse = await axios.delete(
					`${process.env.REACT_APP_ROOT_URL}/api/deleteExpenses/${id}`,
				);
				console.log(deleteReponse);
				const postReponse = await axios.post(
					`${process.env.REACT_APP_ROOT_URL}/api/postExpenses/${userId}`,
					{ expenseList },
				);
	...
	const handleRemoveExpense = (expenseIndex, expenseTypeIndex) => {
		setExpenseList((prevExpenseList) => {
			const newExpenseList = [...prevExpenseList];
			newExpenseList[expenseTypeIndex].expense = newExpenseList[
				expenseTypeIndex
			].expense.filter((_, index) => index !== expenseIndex);
			return newExpenseList;
		});
	};
	...
	return (
		<div>
			<div
				...
				style={{ color: "white", color: "#014c91", cursor: "pointer" }}
			>
				...
				<Form noValidate validated={validated} onSubmit={handleSubmit}>
					<Modal.Body>
						{expenseList.map((expenseType, index) => (
							<Card
								className="mt-4"
								style={{ width: "100%", padding: "20px", color: "#014c91" }}
							>
											...
											<Col lg="2">
												<Form.Group controlId="description">
													<Form.Control
														type="date"
														name="expense_date"
														value={expenseType.expense[0].expense_date}
											...
											<Col lg="2">
												<Form.Group controlId="description">
													<Form.Control
														type="text"
														name="name"
														value={expenseType.expense[0].name}
														onChange={(e) => handleChange(e, 0, index)}
														required
													/>
													<Form.Control.Feedback type="invalid">
											...
											<Col lg="3">
												<Form.Group controlId="description">
													<Form.Control
														type="text"
														name="description"
														value={expenseType.expense[0].description}
														onChange={(e) => handleChange(e, 0, index)}
													/>
													<Form.Control.Feedback type="invalid">
												...
												<Form.Group controlId="srp">
													<InputGroup>
														...
														<Form.Control
															...
															min="0"
															name="amount"
															value={expenseType.expense[0].amount}
															onChange={(e) => handleChange(e, 0, index)}
															required
												...
												<Col lg="2">
													<Form.Group controlId="description">
														<Form.Control
															type="date"
															name="expense_date"
															value={subexpense.expense_date}
															onChange={(e) => handleChange(e, subindex, index)}
															required
														/>
														<Form.Control.Feedback type="invalid">
															Please provide expense date
