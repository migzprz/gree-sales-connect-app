...
const EditOcularModal = ({ id, date, onClose }) => {
	...
	const {
		...
		city,
		barangay,
	} = useAddressFilter(editData, setEditData);
	...
	//         try {
	//             const technicianResponse = await axios.get('${process.env.REACT_APP_ROOT_URL}/api/getTechnicians/')
			...
			try {
				...
				if (output.date && output.time) {
					...
					output = {
						...output,
						ocular_date:
							(editData.date ? editData.date : record.ocular_date.split("T")[0]) +
							(editData.time
								? "T" + editData.time
								: "T" + record.ocular_date.split("T")[1]),
						date: "",
						time: "",
					};
				}
				...
				const updateResponse = axios.patch(
					`${process.env.REACT_APP_ROOT_URL}/api/editOcularById/${id}`,
					output,
				);
				...
				<Form noValidate validated={validated} onSubmit={handleSubmit}>
					<Modal.Body style={{ color: "#014c91", backgroundColor: "#E5EDF4" }}>
						<Row>
							...
							<Col lg="3">{record.client_name}</Col>
						</Row>
						...
						<Row>
							...
							<Col lg="3">{record.companyName}</Col>
						</Row>
						...
						<Row>
							...
							<Col lg="3">{record.contactNumber}</Col>
						</Row>
						...
						<Row className="mt-3">
							<Col lg="4">
								<Form.Group controlId="unitNo">
									...
									<Form.Control
										type="text"
										placeholder={record ? record.addr_bldg_no : ""}
										name="addr_bldg_no"
							...
							<Col lg="4">
								<Form.Group controlId="street">
									...
									<Form.Control
										type="text"
										placeholder={record ? record.addr_street_name : ""}
							...
							<Col lg="3">
								<Form.Group controlId="zipcode">
									...
									<Form.Control
										pattern="[0-9]{4}"
										placeholder={record ? record.zipcode : ""}
										type="text"
										name="zipcode"
										onChange={handleChange}
									/>
						...
						<Row className="mt-2">
							<Col lg="3">
								<Form.Group controlId="region">
									...
									>
										{region
											.filter((reg) => reg.region_id === record.addr_region_id)
											.map((reg) => (
												<option value="">{reg.description + " (DEFAULT)"}</option>
											))}
										{region.map((reg, index) => (
											<option key={index} value={reg.region_id}>
												{reg.description}
											</option>
										))}
									</Form.Control>
							...
							<Col lg="3">
								<Form.Group controlId="province">
									...
									<Form.Control
										as="select"
										onChange={handleChange}
										name="addr_province_id"
										disabled={!(editData.addr_region_id !== "" && isChanged)}
										required={editData.addr_region_id !== "" && isChanged}
									>
										{filteredProvince.length === 0 &&
											province
												.filter((pro) => pro.province_id === record.addr_province_id)
												.map((pro) => <option value="">{pro.name}</option>)}
							...
							<Col lg="3">
								<Form.Group controlId="city">
									...
									<Form.Control
										as="select"
										onChange={handleChange}
										name="addr_municipality_id"
										disabled={
											!(editData.addr_region_id !== "" && isChanged) ||
											editData.addr_province_id === ""
										}
										required={editData.addr_region_id !== "" && isChanged}
									>
										{filteredCity.length === 0 &&
											!editData.addr_region_id &&
											city
												.filter(
													(cit) => cit.municipality_id === record.addr_municipality_id,
												)
												.map((cit) => <option value="">{cit.name}</option>)}
										{editData.addr_region_id && <option value="">Select</option>}
										{filteredCity.map((cit, index) => (
											<option key={index} value={cit.municipality_id}>
												{cit.name}
											</option>
										))}
									</Form.Control>
							...
							<Col lg="3">
								<Form.Group controlId="barangay">
									...
									<Form.Control
										as="select"
										onChange={handleChange}
										name="addr_barangay_id"
										disabled={
											!(editData.addr_region_id !== "" && isChanged) ||
											editData.addr_municipality_id === ""
										}
										required={editData.addr_region_id !== "" && isChanged}
									>
										{filteredBarangay.length === 0 &&
											!editData.addr_region_id &&
											barangay
												.filter((bar) => bar.barangay_id === record.addr_barangay_id)
												.map((bar) => <option value="">{bar.name}</option>)}
										{editData.addr_region_id && <option value="">Select</option>}
										{filteredBarangay.map((bar, index) => (
											<option key={index} value={bar.barangay_id}>
												{bar.name}
											</option>
										))}
									</Form.Control>
							...
							<Col lg="3">
								<Form.Group controlId="technician">
									...
									<Form.Control
										...
										onChange={handleChange}
										name="technician_id"
										required={record.date !== null || record.time !== null}
									>
										<option value="">Select</option>
										{technicians.map((tec, index) => (
											<option key={index} value={tec.technician_id}>
												{tec.complete_name}
											</option>
										))}
									</Form.Control>
