...
const EditWarrantyServiceModal = ({
	service_id,
	id,
	type,
	is_completed,
			...
			if (type === "inspection") {
				try {
					const patchResponse = await axios.patch(
						`${process.env.REACT_APP_ROOT_URL}/api/updateInspection`,
						data,
					);
				...
				try {
					const patchResponse = await axios.patch(
						`${process.env.REACT_APP_ROOT_URL}/api/updateService`,
						data,
					);
					console.log(patchResponse);
		...
		<div>
			{is_completed ? (
				...
				<div
					...
					style={{ color: "#014c91", cursor: "pointer" }}
				>
