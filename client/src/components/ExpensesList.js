		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getExpenses/`,
				);
				setExpenseData(response.data);
				const response2 = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getYearExpenses/`,
				);
													...
													<React.Fragment key={expense.expense_id}>
														<tr style={{ borderRadius: "20px", padding: "10px" }}>
															<td style={{ color: "#014c91" }}>
																<Link
																	to={`/viewexpensedetails/${expense.expense_id}`}
																	style={{ color: "#014c91" }}
																>
																	{formatDate(expense.date_created)}
																</Link>
															</td>
