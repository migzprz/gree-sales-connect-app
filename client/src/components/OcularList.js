...
const OcularList = () => {
		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getOculars/`,
				);
	...
	const renderDropdown = (index, id, date) => {
		if (index === activeDropdown) {
			return (
				...
				>
					<Dropdown.Item href={`generatequotation/${id}?type=convertocular`}>
						Generate Quotation
					</Dropdown.Item>
					<EditOcularModal
						id={id}
	...
	if (sortOption === "date-asc") {
		sortedOculars.sort(
			(a, b) => new Date(a.ocular_date) - new Date(b.ocular_date),
		);
	} else if (sortOption === "date-desc") {
		sortedOculars.sort(
			(a, b) => new Date(b.ocular_date) - new Date(a.ocular_date),
		);
	...
	const filteredOculars = sortedOculars.filter(
		(ocular) =>
			(filterOption === "" ||
				ocular.technician_name.toString() === filterOption) &&
			(ocular.client_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
				(ocular.company_name &&
					ocular.company_name.toLowerCase().includes(searchTerm.toLowerCase())) || // Null check for company_name
						...
						<Col lg="4">
							...
							>
								...
								>
									...
									{ocularData &&
										[...new Set(ocularData.map((item) => item.technician_name))].map(
											(technician, index) => (
											...
											<React.Fragment key={ocular.ocular_id}>
												<tr style={{ borderRadius: "20px", padding: "10px" }}>
													...
													<td style={{ color: "#014c91" }}>
														{formatDate(ocular.ocular_date)}
													</td>
													<td style={{ color: "#014c91" }}>
														{new Date(ocular.ocular_date).toLocaleTimeString([], {
															hour: "2-digit",
															minute: "2-digit",
														})}
													</td>
