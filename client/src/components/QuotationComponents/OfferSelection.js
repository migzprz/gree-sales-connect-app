...
const OfferSelection = ({ offerList, onOfferSubmission, type, id }) => {
		...
		if (type === "edit") {
			const fetchData = async () => {
				try {
					const res = (
						await axios.get(
							`${process.env.REACT_APP_ROOT_URL}/api/getQuotationDetailsById/${id}`,
						)
					).data;
	...
	const handleItemListChange = (event, index, property) => {
		...
		setItemList((prevItemList) => {
			...
			updatedItemList[index]["totalPrice"] =
				updatedItemList[index]["discPrice"] * updatedItemList[index]["quantity"];
			...
			for (const key in offerList) {
				...
				if (Array.isArray(offerList[key])) {
					offerList[key].forEach((offer) => {
						...
						if (key === "products") {
							matches =
								offer.display.toLowerCase().includes(searchTerm.toLowerCase()) ||
								offer.unit_model.toLowerCase().includes(searchTerm.toLowerCase());
						} else if (key === "services") {
							matches = offer.description
								.toLowerCase()
					...
					{itemList.length > 0 && (
						<div className="mt-auto">
							...
							>
								{isFullView ? (
									<>
										{React.createElement(FaEyeSlash, {
											size: 18,
			...
			<Row>
				...
				{itemList.length === 0 ? (
					<Col lg="12">
											...
											{filteredOffers.products.map((offer, index) => (
												<Col className="mt-3" lg="2" key={index}>
													<Card
														...
														onClick={() => handleAddToItemList(offer)}
													>
														<Card.Title>{offer.display}</Card.Title>
														<Card.Text>
															{offer.unit_model} <br />
															<strong>₱ {formatNumber(offer.product_srp)} </strong>
															<br />
											...
											{filteredOffers.services.map((offer, index) => (
												<Col className="mt-3" lg="2" key={index}>
													...
													>
														...
														<Card.Text>
															--- <br />
															<strong>₱ {formatNumber(offer.service_srp)} </strong>
															<br />
											...
											{filteredOffers.parts.map((offer, index) => (
												<Col className="mt-3" lg="2" key={index}>
													...
													>
														...
														<Card.Text>
															{offer.name} <br />
															<strong>₱ {formatNumber(offer.parts_srp)} </strong>
															<br />
															{offer.unit.toUpperCase()}
														</Card.Text>
													</Card>
						...
						{filteredOffers && (
							<>
								{filteredOffers.products.length === 0 &&
									filteredOffers.services.length === 0 &&
									filteredOffers.parts.length === 0 && (
										<Card
											style={{
												borderRadius: "20px",
												marginTop: "20px",
					...
					<>
						...
						<Col
							lg={isFullView ? "7" : "12"}
							style={!isFullView ? { transition: "all 0.2s ease" } : {}}
						>
							<div
								className="mt-3"
								style={{
												...
												<tbody>
													{itemList.map((item, index) => (
														<tr key={index} style={{ borderRadius: "20px", padding: "10px" }}>
															<td style={{ color: "#014c91" }}>
																<Form.Group controlId={`qty-${index}`}>
																	<Form.Control
																		...
																		required
																		value={item.quantity}
																		onChange={(e) => handleItemListChange(e, index, "quantity")}
																	/>
																</Form.Group>
															</td>
															<td style={{ color: "#014c91" }}>
															...
															<td style={{ color: "#014c91" }}>₱ {formatItemList(item)}</td>
															<td style={{ color: "#014c91" }}>
