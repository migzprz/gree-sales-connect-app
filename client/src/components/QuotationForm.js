		...
		if (type === "view") {
			...
			const fetchData = async () => {
				try {
					const res = (
						await axios.get(
							`${process.env.REACT_APP_ROOT_URL}/api/getQuotationDetailsById/${quoId}`,
						)
					).data;
					console.log(res);
					setClientData(res.client[0]);
		...
		try {
			if (id) {
				...
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getQuoClientIdByOcularId/${id}`,
				);

				client_id = response.data[0].quotation_client_id;
			} else if (salesId) {
				console.log(
					"Adding additional quotation for existing sales record, searching quotation client id",
				);
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getQuoClientIdBySalesId/${salesId}`,
				);
				...
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getQuoClientIdByQuoId/${quoId}`,
				);
				...
				const response = await axios.post(
					`${process.env.REACT_APP_ROOT_URL}/api/postOcular/0`,
					clientData,
				);
			...
			const postReponse = await axios.post(
				`${process.env.REACT_APP_ROOT_URL}/api/postQuotation/`,
				data,
			);
			...
			if (type === "edit") {
				...
				const cancelResponse = await axios.patch(
					`${process.env.REACT_APP_ROOT_URL}/api/cancelQuotation/${quoId}`,
				);
			...
			if (salesId) {
				navigate(
					`/generateinvoice?id=${postReponse.data.quotation_id}&type=add&sales=${salesId}`,
				);
			} else {
			...
			const fetchData = async () => {
				try {
					const res = await axios.get(
						`${process.env.REACT_APP_ROOT_URL}/api/getOcular/${id}`,
					);
			...
			{loading ? (
				...
				>
					...
					{selectionType === "offer" && (
						<OfferSelection
							offerList={offer}
							onOfferSubmission={handleOfferSubmission}
							type={type}
							id={quoId}
						/>
					)}
					{selectionType === "client" && (
						<ClientSelection onClientSubmission={handleClientSubmission} />
					)}
					{selectionType === "terms" && (
						<TermsAndConditions
							onTermsSubmission={handleGetTermsData}
							termsData={termsData}
						/>
					)}
