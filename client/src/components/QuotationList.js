...
const QuotationList = () => {
		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getQuotations/`,
				);
	...
	const checkExpiry = async (list) => {
		list.forEach(async (data) => {
			...
			if (currDate >= dateCreated && data.is_cancelled === 0) {
				console.log(
					"quotation record ",
					String(data.quotation_id).padStart(4, "0"),
					" expired. Patching database",
				);
				try {
					const res = await axios.patch(
						`${process.env.REACT_APP_ROOT_URL}/api/cancelQuotation/${data.quotation_id}`,
					);
				} catch (error) {
					console.log(error);
				}
			} else {
				console.log("record has not expired");
			}
		});
	...
	const renderDropdown = (index, id) => {
		if (index === activeDropdown) {
			return (
				<Dropdown.Menu
					style={{ position: "absolute", right: "0", left: "auto", top: "0px" }}
				>
					<Dropdown.Item>
						<Link
							to={`/generatequotation?id=${id}&type=edit`}
		...
		(quotation) =>
			...
			(quotation.client_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
				(quotation.company_name &&
					quotation.company_name.toLowerCase().includes(searchTerm.toLowerCase())) || // Null check for company_name
										...
										{currentItems.map((quotation, index) => (
											<React.Fragment key={quotation.quotation_id}>
												<tr style={{ borderRadius: "20px", padding: "10px" }}>
													<td>
														<Link
															to={`/generatequotation/${quotation.quotation_id}?type=view`}
															style={{ color: "#014c91" }}
														>
															#{String(quotation.quotation_id).padStart(4, "0")}
														</Link>
													</td>
													...
													<td style={{ color: "#014c91" }}>
														...
														{new Date(quotation.date_created).toLocaleTimeString([], {
															hour: "2-digit",
															minute: "2-digit",
														})}
													</td>
													<td style={{ color: "#014c91" }}>
														₱ {formatNumber(quotation.totalPrice)}
													</td>
													<td style={{ color: "#014c91" }}>
