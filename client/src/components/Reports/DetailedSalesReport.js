...
const DetailedSalesReport = () => {
	const { syear, smonth, sday, eyear, emonth, eday, id, producttype } =
		useParams();
	...
	useEffect(() => {
		const fetchProductOverviewData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getDetailedReportSalesProductsTotal/${id}/${syear}/${smonth}/${sday}/${eyear}/${emonth}/${eday}`,
				);
				...
				const responsePrev = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getDetailedReportSalesProductsTotal/${id}/${syearPrev}/${smonthPrev}/${sdayPrev}/${eyearPrev}/${emonthPrev}/${edayPrev}`,
				);
				...
				const growth = (
					((totalSales - totalSalesPrev) / totalSalesPrev) *
					100
				).toFixed(2);
				let growthWithSign;
				...
				if (!isFinite(growth)) {
					...
					growthWithSign =
						(growth > 0 ? `+${growthLimited}` : `${growthLimited}`) + "%";
				}
		...
		const fetchProductsData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getDetailedReportSalesProducts/${id}/${syear}/${smonth}/${sday}/${eyear}/${emonth}/${eday}`,
				);
		...
		const fetchPartOverviewData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getDetailedReportSalesPartsTotal/${id}/${syear}/${smonth}/${sday}/${eyear}/${emonth}/${eday}`,
				);
				console.log(response.data[0].total);
				const totalSales = response.data[0].total;
				...
				// Fetch report sales total for the previous period
				const responsePrev = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getDetailedReportSalesPartsTotal/${id}/${syearPrev}/${smonthPrev}/${sdayPrev}/${eyearPrev}/${emonthPrev}/${edayPrev}`,
				);
				console.log(responsePrev.data[0].total);
				const totalSalesPrev = responsePrev.data[0].total;
		...
		const fetchPartsData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getDetailedReportSalesParts/${id}/${syear}/${smonth}/${sday}/${eyear}/${emonth}/${eday}`,
				);
				console.log(response.data);
		...
		const fetchServiceOverviewData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getDetailedReportSalesServicesTotal/${id}/${syear}/${smonth}/${sday}/${eyear}/${emonth}/${eday}`,
				);
				console.log(response.data[0].total);
				const totalSales = response.data[0].total;
				...
				// Fetch report sales total for the previous period
				const responsePrev = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getDetailedReportSalesServicesTotal/${id}/${syearPrev}/${smonthPrev}/${sdayPrev}/${eyearPrev}/${emonthPrev}/${edayPrev}`,
				);
				console.log(responsePrev.data[0].total);
				const totalSalesPrev = responsePrev.data[0].total;
		...
		const fetchServicesData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getDetailedReportSalesServices/${id}/${syear}/${smonth}/${sday}/${eyear}/${emonth}/${eday}`,
				);
				console.log(response.data);
	...
	const downloadPDF = () => {
		...
		Promise.all(promises).then((canvases) => {
			...
			setLoader(false);
			doc.save(
				`detailed_sales_report_${formatDate(reportDetails.date_generated)}.pdf`,
			);
		});
	};
	...
	return (
		...
		>
			...
			{pages.map((item, index) => (
				<Card
					className={`quotation-file mt-2 page-${index}`}
						...
						<tbody>
							<tr>
								<td colSpan="5" style={{ padding: "0", border: "none" }}>
									...
									<strong>
										{" "}
										{formatDate(reportDetails.date_generated)}{" "}
										{new Date(reportDetails.date_generated).toLocaleTimeString([], {
											hour: "2-digit",
											minute: "2-digit",
										})}
									</strong>{" "}
							...
							<tr>
								<td colSpan="5" style={{ padding: "0", border: "none" }}>
									Generated by <strong>{reportDetails.user}</strong>{" "}
								</td>
							</tr>
							<tr>
								<td colSpan="5" style={{ padding: "0", border: "none" }}>
									Period Covered:{" "}
									<strong>
										{formatDate(reportDetails.start_date)} to{" "}
										{formatDate(reportDetails.end_date)}
									</strong>{" "}
								</td>
							</tr>
							{index === 0 ? (
								<>
									...
									<tr>
										<td colSpan="3" style={{ padding: "3px", border: "1px solid black" }}>
											Sales Growth (vs. Last {periodDays} Days)
										</td>
										<td
											colSpan="2"
											style={{
												...
												border: "1px solid black",
												textAlign: "right",
												color: salesGrowth.startsWith("+") ? "green" : "red",
											}}
										>
											<strong>{salesGrowth} </strong>
										</td>
									</tr>
								</>
							) : null}
							...
							{item.map((product, index) => (
								<tr key={index}>
									...
									>
										₱ {formatNumber(product.discounted_price_each)}
									</td>
									<td
										style={{
											padding: "3px",
