...
const ProfitStatement = () => {
		...
		const fetchProductsData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getProfitStatement/${syear}/${smonth}/${sday}/${eyear}/${emonth}/${eday}`,
				);
	...
	useEffect(() => {
		if (productData.length === 0) {
			...
			const sumRevenue = productData
				.filter((product) => [1, 2, 3, 4].includes(product.type))
				.reduce((total, product) => total + product.total, 0);
			const sumExpense = productData
				.filter((product) => [5, 6].includes(product.type))
				.reduce((total, product) => total + product.total, 0);
	...
	const downloadPDF = () => {
		...
		Promise.all(promises).then((canvases) => {
			...
			setLoader(false);
			doc.save(
				`profit_and_loss_statement_${formatDate(reportDetails.date_generated)}.pdf`,
			);
		});
	};
								...
								<>
									...
									{item.some((product) => product.type === 1) && (
										<>
											<tr>
												<td
													style={{ padding: "3px", border: "1px solid black", width: "60%" }}
												>
									...
									{(item.some((product) => product.type === 5 || product.type === 6) ||
										(pages[index + 1] &&
											!pages[index + 1].some((product) =>
												[1, 2, 3, 4].includes(product.type),
											))) && (
										<tr>
											<td
												colSpan="2"
									...
									{index === pages.length - 1 ? (
										<>
											...
											<tr>
												...
												>
													₱ {formatNumber(totalRevenue - totalExpense - totalTax)}
												</td>
											</tr>
										</>
									) : null}
