...
const QuotationConversionReport = () => {
		...
		const fetchQuotationsData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getQuotationConversionReport/${syear}/${smonth}/${sday}/${eyear}/${emonth}/${eday}`,
				);
	...
	const downloadPDF = () => {
		...
		Promise.all(promises).then((canvases) => {
			...
			setLoader(false);
			doc.save(
				`quotation_conversion_report_${formatDate(reportDetails.date_generated)}.pdf`,
			);
		});
	};
							...
							{item.some((product) => product.type === 2) && (
								<>
									...
									<tr>
										...
										>
											<strong>Potential Sale Amount</strong>
										</td>
									</tr>
									{item
							...
							{item.some((product) => product.type === 3) && (
								<>
									...
									<tr>
										...
										>
											<strong>Cancelled Sale Amount</strong>
										</td>
									</tr>
									{item
