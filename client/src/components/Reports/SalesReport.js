...
const SalesReport = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getReportSalesTotal/${syear}/${smonth}/${sday}/${eyear}/${emonth}/${eday}`,
				);
				...
				const responsePrev = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getReportSalesTotal/${syearPrev}/${smonthPrev}/${sdayPrev}/${eyearPrev}/${emonthPrev}/${edayPrev}`,
				);
		...
		const fetchProductsData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getReportSalesProducts/${syear}/${smonth}/${sday}/${eyear}/${emonth}/${eday}`,
				);
						...
						<tbody>
							...
							{index === 0 ? (
								<>
									...
									<tr>
										<td colSpan="2" style={{ padding: "3px", border: "1px solid black" }}>
											Total Sales Revenue{" "}
										</td>
										<td
											colSpan="2"
							...
							{item.some((product) => product.type === 1) && (
								<>
									...
									<tr>
										<td
											style={{ padding: "3px", border: "1px solid black", width: "50%" }}
										>
											<strong>Window-Type Product</strong>
										</td>
										<td
											style={{
												padding: "3px",
									...
									{item
										...
										.map((product, index) => (
											<tr key={index}>
												...
												>
													₱ {formatNumber(product.average_amount)}
												</td>
												<td
													style={{
														padding: "3px",
								...
								<>
									...
									<tr>
										<td
											style={{ padding: "3px", border: "1px solid black", width: "50%" }}
										>
											<strong>Split-Type Product</strong>
										</td>
										<td
											style={{
												padding: "3px",
