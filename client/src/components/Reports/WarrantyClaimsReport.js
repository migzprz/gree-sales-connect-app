...
const WarrantyClaimsReport = () => {
		...
		const fetchProductsData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getWarrantyClaimsReport/${syear}/${smonth}/${sday}/${eyear}/${emonth}/${eday}`,
				);
	...
	const downloadPDF = () => {
		...
		Promise.all(promises).then((canvases) => {
			...
			setLoader(false);
			doc.save(
				`warranty_claims_report_${formatDate(reportDetails.date_generated)}.pdf`,
			);
		});
	};
							...
							{item.some((product) => product.type === 1) && (
								<>
									...
									<tr>
										...
										>
											<strong>Date of Claiming</strong>
										</td>
										<td
											style={{
												padding: "3px",
