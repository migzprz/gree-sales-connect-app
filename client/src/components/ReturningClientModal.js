...
const ReturningClientModal = ({ formData, setFormData }) => {
		...
		const fetchData = async () => {
			try {
				const clientResponse = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getAllClients/`,
				);
	...
	const filteredClients = clients.filter(
		(client) =>
			...
			(client.company_name &&
				client.company_name.toLowerCase().includes(searchTerm.toLowerCase())) || // Null check for company_name
								...
								<Table bordered hover>
									...
									<tbody>
										{filteredClients.map((client, index) => (
											<tr
												key={index}
												onClick={() => handleOnClick(index, client)}
