...
const SaleConvertDetails = () => {
		...
		const fetchData = async () => {
			try {
				const res = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/doesQuotationContainOnlyService/${id}`,
				);
				const res2 = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/quotationTotalPrice/${id}`,
				);
	...
	useEffect(() => {
		try {
			const fetchdata = async () => {
				const res = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getModeOfPayments`,
				);
				// const res2 = await axios.get(`${process.env.REACT_APP_ROOT_URL}/api/getTechnicians`)
				setMop(res.data);
	...
	const [inputDateTimeInstallation, setInputDateTimeInstallation] =
		useState(null);
	const [inputDateTimeService, setInputDateTimeService] = useState(null);
	const { technicians, mod } = useAvailableTechnicians(
		inputDateTimeInstallation,
	);
	const { technicians: technicians2, mod: mod2 } =
		useAvailableTechnicians(inputDateTimeService);
	...
	useEffect(() => {
		...
		if (installation.installationSDate && installation.installationSTime) {
			...
			const newInputDateTime =
				installation.installationSDate + "T" + installation.installationSTime;
		...
		if (form.checkValidity() === true) {
			try {
				const res = await axios.post(
					`${process.env.REACT_APP_ROOT_URL}/api/convertToSale/${type === "add" ? "add" : "new"}`,
					{ id, payment, delivery, installation, services, sales, login_id },
				);
				console.log(res);
	...
	const handleCheckClick = (e) => {
		...
		if (isInstallCheck) {
			console.log(
				"Setting installation schedule to be the same as the delivery schedule",
			);
			...
			if (isSame) {
				...
				// clear delivery date and time from the form
				setInstallation((prev) => {
			...
			console.log(
				"Setting service schedule to be the same as the delivery schedule",
			);
					...
					<Col lg="2">
						<Form.Group controlId="paymentMode">
							...
							<Form.Control as="select" name="mop_id" onChange={handleChange} required>
								...
								{mop.map((m, index) => (
									<option key={index} value={m.mop_id}>
										{m.name}
									</option>
								))}
							</Form.Control>
						...
						<Form.Group controlId="amount">
							...
							<InputGroup>
								...
								<Form.Control
									...
									disabled={payment.isInstallment === "0"}
						...
						<Row className="mt-3">
							...
							<Col lg="2">
								<Form.Group controlId="date">
									<Form.Label>
										{delivery.isPickup === "1" ? "Pickup " : "Delivery "} Date
									</Form.Label>
									<Form.Control
										type="date"
										name="deliveryDate"
							...
							<Col lg="2">
								<Form.Group controlId="time">
									<Form.Label>
										{delivery.isPickup === "1" ? "Pickup " : "Delivery "} Time
									</Form.Label>
									<Form.Control
										type="time"
										name="deliveryTime"
						...
						<Row className="mt-3">
							...
							<Col lg="2">
								<Form.Group controlId="installstartdate">
									...
									<Form.Control
										type="date"
										name="installationSDate"
										onChange={handleChange}
										defaultValue={installation.installationSDate}
										required
									/>
									<Form.Control.Feedback type="invalid">
										Please choose a valid date.
									...
									{delivery && delivery.deliveryDate && delivery.deliveryTime ? (
										<Form.Check
											...
											name="installCheck"
											checked={installationAsDelivery}
							...
							<Col lg="2">
								<Form.Group controlId="installstarttime">
									...
									<Form.Control
										type="time"
										name="installationSTime"
										onChange={handleChange}
										defaultValue={installation.installationSTime}
										required
									/>
									<Form.Control.Feedback type="invalid">
										Please choose a valid time.
									...
									<Form.Group controlId="installationTechnician">
										...
										>
											...
											{technicians.map((t, index) => (
												<option key={index} value={t.technician_id}>
													{t.complete_name}
												</option>
											))}
										</Form.Control>
										{technicians.length === 0 ? (
											<p style={{ color: "red" }}>No technicians are available</p>
										) : mod ? (
											<p>Some technicians are unavailable</p>
										) : null}
									</Form.Group>
								</Col>
							) : null}
							...
							<Col lg="2">
								<Form.Group controlId="installstartdate">
									...
									<Form.Control
										...
										name="serviceDate"
										onChange={handleChange}
										defaultValue={services.serviceDate}
										required
									/>
									<Form.Control.Feedback type="invalid">
										Please choose a valid date.
							...
							<Col lg="2">
								<Form.Group controlId="installstarttime">
									...
									<Form.Control
										...
										name="serviceTime"
										onChange={handleChange}
										defaultValue={services.serviceTime}
										required
									/>
									<Form.Control.Feedback type="invalid">
										Please choose a valid time.
							...
							<Col lg="2">
								<Form.Group controlId="paymentMode">
									...
									<Form.Control
										...
										disabled={!services.serviceDate || !services.serviceTime}
										onChange={handleChange}
										defaultValue={services.serviceTechnician}
										required
									>
										<option value=""> Select </option>
