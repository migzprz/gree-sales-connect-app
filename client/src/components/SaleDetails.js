		...
		const fetchData = async () => {
			if (id !== null) {
				try {
					const response = (
						await axios.get(
							`${process.env.REACT_APP_ROOT_URL}/api/getSalesById/${id}`,
						)
					).data;
				...
				>
					...
					<Row>
						<Col>
							...
							services.length === 0 &&
							quotations.reduce((sum, q) => sum + q.totalPrice, 0) -
							...
							>
								<Card
									style={{
										padding: "15px",
										borderRadius: "20px",
										color: deliveries.length > 0 ? "#014c91" : "#008000",
										background: deliveries.length > 0 ? "" : "#E7FFE2",
										height: "100%",
									}}
								>
									<Row>
										<Col lg="9">
											<h3>
												{React.createElement(FaTruck, {
													size: 25,
													style: { marginRight: "5px", marginBottom: "5px" },
												})}
												{deliveries.length > 0 && deliveries[0].is_pickup
													? "Pick Up"
													: "Delivery"}
											</h3>
										</Col>
									...
									{deliveries && deliveries.length > 0 ? (
										<>
											<Row>
												<Col>
													Date:{" "}
													<strong>
														{new Date(deliveries[0].delivery_date).toLocaleDateString()}
													</strong>
												</Col>
											</Row>
											<Row>
												<Col>
													Time:{" "}
													<strong>
														{new Date(deliveries[0].delivery_date).toLocaleTimeString()}
													</strong>
												</Col>
											</Row>
											<Row>
												<Col>
													{deliveries.length > 1
														? `There ${deliveries.length - 1 > 1 ? "are" : "is"} ${deliveries.length - 1} more ${installations.length - 1 > 1 ? "deliveries" : "delivery"} left`
														: null}
												</Col>
											</Row>
											<Row className="mt-2">
												<Col>
													<CompleteServiceModal
														...
														id={deliveries[0].delivery_id}
														refId={String(detail.sales_id).padStart(4, "0")}
													/>
												</Col>
											</Row>
										</>
										...
										<Row>
											<Col>No upcoming deliveries</Col>
										</Row>
									)}
								</Card>
								...
								<Card
									...
									style={{
										padding: "15px",
										borderRadius: "20px",
										color: installations.length > 0 ? "#014c91" : "#008000",
										background: installations.length > 0 ? "" : "#E7FFE2",
										height: "100%",
									}}
								>
									<Row>
										<Col lg="9">
											<h3>
												{React.createElement(FaScrewdriver, {
													size: 25,
													style: { marginRight: "5px", marginBottom: "5px" },
												})}
												Installation
									...
									{installations && installations.length > 0 ? (
										<>
											...
											<Row>
												<Col>
													...
													<strong>
														{new Date(
															installations[0].start_installation_date,
														).toLocaleDateString()}
													</strong>
												</Col>
											</Row>
											...
											<Row>
												<Col>
													{installations.length > 1
														? `There ${installations.length - 1 > 1 ? "are" : "is"} ${installations.length - 1} more ${installations.length - 1 > 1 ? "installations" : "installation"} left`
														: null}
												</Col>
											</Row>
											{deliveries.length < installations.length && (
												<Row className="mt-2">
													<Col>
														<CompleteServiceModal
															type={"installation"}
															id={installations[0].installation_id}
															refId={String(detail.sales_id).padStart(4, "0")}
														/>
													</Col>
												</Row>
										...
										<Row>
											<Col>No upcoming installations</Col>
										</Row>
									)}
								</Card>
									...
									<>
										<Card
											...
											style={{
												padding: "15px",
												borderRadius: "20px",
												color: services.length > 0 ? "#014c91" : "#008000",
												background: services.length > 0 ? "" : "#E7FFE2",
											}}
										>
											<Row>
												<Col lg="9">
													<h3>
														{React.createElement(FaToolbox, {
															size: 25,
															style: { marginRight: "5px", marginBottom: "5px" },
														})}
														Service
											...
											<Row>
												<Col>
													Date:{" "}
													<strong>
														{new Date(services[0].service_date).toLocaleDateString()}
													</strong>
												</Col>
											</Row>
											<Row>
												<Col>
													Time:{" "}
													<strong>
														{new Date(services[0].service_date).toLocaleTimeString()}
													</strong>
												</Col>
											</Row>
											<Row>
											...
											<Row>
												<Col>
													{services.length > 1
														? `There ${services.length - 1 > 1 ? "are" : "is"} ${services.length - 1} more ${services.length - 1 > 1 ? "services" : "service"} left`
														: null}
												</Col>
											</Row>
											...
											<Row className="mt-2">
												<Col>
													<CompleteServiceModal
														type={"service"}
														id={services[0].service_schedule_id}
														refId={String(detail.sales_id).padStart(4, "0")}
													/>
												</Col>
											</Row>
							...
							>
								...
								>
									<Row>
										<Col lg="9">
											<h3>
												{React.createElement(FaMoneyBillWave, {
													size: 25,
													style: { marginRight: "5px", marginBottom: "5px" },
												})}
												Payment History
											...
											<Table className="w-full">
												...
												<tbody>
													<React.Fragment>
														...
														<tr>
															<td colSpan="3" style={{ color: "#014c91", textAlign: "right" }}>
																Total Amount Paid
															</td>
															<td style={{ color: "#014c91" }}>
																<strong>
																	₱
																	{formatNumber(
																		payments.reduce((sum, item) => item.amount + sum, 0),
																	)}{" "}
																</strong>
															</td>
														</tr>
							...
							>
								...
								>
									<Row>
										<Col lg="9">
											<h3>
												{React.createElement(FaShoppingBag, {
													size: 25,
													style: { marginRight: "5px", marginBottom: "5px" },
												})}
												Purchase Summary
														...
														<tr style={{ borderRadius: "20px", padding: "10px" }}>
															...
															<td style={{ color: "#014c91" }}>
																<Link to={`/generatequotation/${q.quotation_id}?type=view`}>
																	...
																	>
																		{React.createElement(FaMagnifyingGlass, {
																			size: 18,
															...
															<td style={{ color: "#014c91" }}>
																<Link
																	to={`/generatequotation/${q.quotation_id}?type=view&purchase=true`}
																>
																	<button
																		className="btn w-40"
																		style={{ color: "white", backgroundColor: "#014c91" }}
