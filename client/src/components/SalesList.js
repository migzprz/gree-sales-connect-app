...
const SalesList = () => {
		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getSales/`,
				);
	...
	if (sortOption === "delivdate-asc") {
		sortedSales.sort(
			(a, b) =>
				new Date(a.latest_delivery_date) - new Date(b.latest_delivery_date),
		);
	} else if (sortOption === "delivdate-desc") {
		sortedSales.sort(
			(a, b) =>
				new Date(b.latest_delivery_date) - new Date(a.latest_delivery_date),
		);
	} else if (sortOption === "installdate-asc") {
		sortedSales.sort(
			(a, b) =>
				new Date(a.latest_installation_date) - new Date(b.latest_installation_date),
		);
	} else if (sortOption === "installdate-desc") {
		sortedSales.sort(
			(a, b) =>
				new Date(b.latest_installation_date) - new Date(a.latest_installation_date),
		);
	} else if (sortOption === "servicedate-asc") {
		sortedSales.sort(
			(a, b) => new Date(a.latest_service_date) - new Date(b.latest_service_date),
		);
	} else if (sortOption === "servicedate-desc") {
		sortedSales.sort(
			(a, b) => new Date(b.latest_service_date) - new Date(a.latest_service_date),
		);
	}
	...
	const filteredSales = sortedSales.filter(
		(sale) =>
			...
			(sale.client_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
				(sale.company_name &&
					sale.company_name.toLowerCase().includes(searchTerm.toLowerCase())) || // Null check for company_name
				...
				>
					...
					<Row>
						...
						<Col lg="3">
							...
							>
								<div
									style={{ backgroundColor: "#014c91", width: "30px", height: "100%" }}
								>
						...
						<Col lg="3">
							...
							>
								...
								>
									...
									<option value="installdate-asc">
										Sort by Installation Date (A-Z)
									</option>
									<option value="installdate-desc">
										Sort by Installation Date (Z-A)
									</option>
					...
					{filteredSales.length > 0 ? (
											...
											<React.Fragment key={sales.sales_id}>
												<tr style={{ borderRadius: "20px", padding: "10px" }}>
													<td style={{ color: "#014c91" }}>
														<Link
															to={`/viewsaledetails?id=${sales.sales_id}`}
															style={{ color: "#014c91" }}
														>
															{String(sales.sales_id).padStart(4, "0")}
														</Link>
													</td>
													...
													<td
														style={{
															color:
																sales.hasDelivery && sales.allDeliveriesCompleted
																	? "#008000"
																	: sales.hasDelivery && !sales.allDeliveriesCompleted
																		? "#DC6601"
																		: "",
													...
													>
														{sales.hasDelivery && sales.allDeliveriesCompleted ? (
															<FaCheck size={20} className="me-1" />
														) : sales.hasDelivery && !sales.allDeliveriesCompleted ? (
														...
														{sales.latest_delivery_date ? (
															<>
																...
																{new Date(sales.latest_delivery_date).toLocaleTimeString([], {
																	hour: "2-digit",
																	minute: "2-digit",
																})}
															</>
													...
													<td
														style={{
															color:
																sales.hasInstallation && sales.allInstallationsCompleted
																	? "#008000"
																	: sales.hasInstallation && !sales.allInstallationsCompleted
																		? "#DC6601"
																		: "",
													...
													>
														...
														{sales.latest_installation_date ? (
															<>
																...
																{new Date(sales.latest_installation_date).toLocaleTimeString(
																	[],
													...
													<td
														style={{
															color:
																sales.hasService && sales.allServicesCompleted
																	? "#008000"
																	: sales.hasService && !sales.allServicesCompleted
																		? "#DC6601"
																		: "",
													...
													>
														...
														{sales.latest_service_date ? (
															<>
																...
																{new Date(sales.latest_service_date).toLocaleTimeString([], {
																	hour: "2-digit",
																	minute: "2-digit",
																})}
															</>
						...
						<Card
							style={{ borderRadius: "20px", marginTop: "20px", textAlign: "center" }}
						>
							<CardBody style={{ padding: "100px", color: "#014c91" }}>
								<h1 className="mt-3">
									{" "}
									<FaSearch size={50} className="me-2" />
									No Sales Found{" "}
								</h1>
							</CardBody>
						</Card>
