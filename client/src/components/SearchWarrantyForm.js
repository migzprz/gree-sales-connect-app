...
const SearchWarrantyForm = () => {
		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getWarrantySearch/`,
				);
	...
	const filteredWarranties = sortedWarranties.filter(
		(warranty) =>
			warranty.client_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
			(warranty.company_name &&
				warranty.company_name.toLowerCase().includes(searchTerm.toLowerCase())) || // Null check for company_name
	...
	const currentItems = filteredWarranties.slice(
		indexOfFirstItem,
		indexOfLastItem,
	);
						...
						<Card style={{ borderRadius: "20px", marginTop: "20px" }}>
							<CardBody>
								<Table>
									...
									<tbody>
										{[...new Set(currentItems.map((item) => item.quotation_id))].map(
											(quotationId, index) => {
												const claimables = currentItems.filter(
													...
													<React.Fragment key={index}>
														{expandedRows.includes(quotationId) ? (
															<tr style={{ borderRadius: "20px", padding: "10px" }}>
																<td
																	style={{ borderBottom: "none", color: "#014c91", width: "9%" }}
																>
																	{claimables[0].client_name}
																</td>
																<td
																	style={{
																		borderBottom: "none",
																...
																>
																	{claimables[0].company_name}
																</td>
																<td style={{ borderBottom: "none", color: "#014c91" }}>
																	{claimables[0].client_number}
																</td>
																<td style={{ borderBottom: "none", color: "#014c91" }}>
																	{claimables[0].email}
																</td>
																...
																>
																	{claimables[0].site_address}
																</td>
																<td style={{ borderBottom: "none", color: "#014c91" }}>
																	{new Date(claimables[0].delivery_date).toLocaleString()}
																</td>
																<td style={{ borderBottom: "none", color: "#014c91" }}>
																	<Link
																		to={`/claimwarranty/${claimables[0].quotation_id}`}
																		className="btn w-60"
															...
															<tr style={{ borderRadius: "20px", padding: "10px" }}>
																<td style={{ color: "#014c91", width: "9%" }}>
																	{claimables[0].client_name}
																</td>
																<td style={{ color: "#014c91", width: "12%" }}>
																	{claimables[0].company_name}
																</td>
																...
																<td style={{ color: "#014c91", width: "20%" }}>
																	{claimables[0].site_address}
																</td>
																<td style={{ color: "#014c91" }}>
																	{new Date(claimables[0].delivery_date).toLocaleString()}
														...
														{expandedRows.includes(quotationId) && (
															<>
																<tr key={index}>
																	<th
																		colSpan="1"
																		style={{ color: "#014c91", padding: "0", border: "none" }}
																	>
																		Unit Model
																	</th>
																	<th
