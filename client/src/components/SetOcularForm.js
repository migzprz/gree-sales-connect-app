...
const SetOcularForm = () => {
	...
	const { region, filteredProvince, filteredCity, filteredBarangay } =
		useAddressFilter(formData, setFormData);

	// fetch location data and stored client and location data for quick fillup feature
	const fetchData = async () => {
		try {
			const locStoredResponse = await axios.get(
				`${process.env.REACT_APP_ROOT_URL}/api/getStoredLocations/`,
			);
		...
		try {
			const clientResponse = await axios.get(
				`${process.env.REACT_APP_ROOT_URL}/api/getContactPerson/`,
			);
		...
		try {
			const companyResponse = await axios.get(
				`${process.env.REACT_APP_ROOT_URL}/api/getCompanies/`,
			);
			...
			try {
				if (formData.client_id === null && !isNew) {
					...
					const postReponse = await axios.post(
						`${process.env.REACT_APP_ROOT_URL}/api/postOcular/1`,
						data,
					);
	...
	return (
		...
		>
			...
			{!isNew && (
				<ReturningClientModal formData={formData} setFormData={setFormData} />
			)}
			...
			<Form noValidate validated={validated} onSubmit={handleSubmit}>
					...
					<Col lg="3">
						<Form.Group controlId="firstName">
							...
							<Form.Control
								type="text"
								disabled={!isNew}
								value={formData.returningClientFirstName || formData.firstName}
					...
					<Col lg="3">
						<Form.Group controlId="lastName">
							...
							<Form.Control
								type="text"
								disabled={!isNew}
								value={formData.returningClientLastName || formData.lastName}
								onChange={handleChange}
								name="lastName"
								required
							/>
					...
					<Col lg="3">
						<Form.Group controlId="companyName">
							...
							<Form.Control
								type="text"
								disabled={!isNew}
								value={formData.returningClientCompanyName || formData.companyName}
								onChange={handleChange}
								name="companyName"
					...
					<Col lg="3">
						<Form.Group controlId="contactNumber">
							...
							<Form.Control
								...
								disabled={!isNew}
								value={formData.returningClientContactNumber || formData.contactNumber}
								onChange={handleChange}
								name="contactNumber"
								required
							/>
					...
					<Col lg="3">
						<Form.Group controlId="email">
							...
							<Form.Control
								type="email"
								disabled={!isNew}
								value={formData.returningClientEmail || formData.email}
								onChange={handleChange}
								name="email"
								required
							/>
					...
					<Col lg="3">
						<Form.Group controlId="tin">
							...
							<Form.Control
								...
								pattern="[0-9-]{9,15}"
								disabled={!isNew}
								value={formData.returningClientCompanyTin || formData.tin}
								placeholder="optional"
								name="tin"
								onChange={handleChange}
							/>
				...
				<Row className="mt-2">
					...
					<Col lg="2">
						<Form.Group controlId="province">
							...
							<Form.Control
								disabled={!formData.addr_region_id}
								as="select"
								onChange={handleChange}
					...
					<Col lg="2">
						<Form.Group controlId="city">
							...
							<Form.Control
								disabled={!formData.addr_province_id}
								as="select"
								onChange={handleChange}
							...
							>
								...
								{filteredCity.map((cit) => (
									<option key={cit.municipality_id} value={cit.municipality_id}>
										{cit.name}
									</option>
								))}
							</Form.Control>
					...
					<Col lg="2">
						<Form.Group controlId="barangay">
							...
							<Form.Control
								disabled={!formData.addr_municipality_id}
								as="select"
								onChange={handleChange}
							...
							>
								...
								{filteredBarangay.map((bar) => (
									<option key={bar.barangay_id} value={bar.barangay_id}>
										{bar.name}
									</option>
								))}
							</Form.Control>
				...
				<Row>
					<Col lg="3">
						...
						>
							{React.createElement(FaCalendarDay, {
								size: 18,
