		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getTechnician/${id}`,
				);
		...
		try {
			if (record.is_active) {
				const response = await axios.patch(
					`${process.env.REACT_APP_ROOT_URL}/api/changeTechnicianState/${id}/0`,
				);
				console.log(response);
			} else {
				const response = await axios.patch(
					`${process.env.REACT_APP_ROOT_URL}/api/changeTechnicianState/${id}/1`,
				);
				console.log(response);
			}
		} catch (error) {
	...
	return (
		<div>
			...
			<Modal show={showModal} onHide={handleCloseModal}>
				...
				>
					<Modal.Title>
						{record.is_active ? "Deactivate Technician" : "Reactivate Technician"}
					</Modal.Title>
				</Modal.Header>
				...
				<Form noValidate validated={validated} onSubmit={handleSubmit}>
					<Modal.Body style={{ color: "#014c91", backgroundColor: "#E5EDF4" }}>
						{record.is_active ? (
							...
							<Row>
								<Col>
									Reinstating <strong> {record.name} </strong> as a technician
									<br />
									Continue?
								</Col>
							</Row>
