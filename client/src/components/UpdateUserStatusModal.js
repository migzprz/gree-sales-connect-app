		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getSystemUser/${id}`,
				);
		...
		try {
			if (record.is_active) {
				const response = await axios.patch(
					`${process.env.REACT_APP_ROOT_URL}/api/changeUserState/${id}/0`,
				);
				console.log(response);
			} else {
				const response = await axios.patch(
					`${process.env.REACT_APP_ROOT_URL}/api/changeUserState/${id}/1`,
				);
				console.log(response);
			}
		} catch (error) {
	...
	return (
		<div>
			...
			<Modal show={showModal} onHide={handleCloseModal}>
				...
				>
					<Modal.Title>
						{record.is_active ? "Deactivate User" : "Reactivate User"}
					</Modal.Title>
				</Modal.Header>
				...
				<Form noValidate validated={validated} onSubmit={handleSubmit}>
					<Modal.Body style={{ color: "#014c91", backgroundColor: "#E5EDF4" }}>
						{record.is_active ? (
							...
							<Row>
								<Col>
									Reinstating access to <strong> {record.name} </strong>
									<br />
									Continue?
								</Col>
							</Row>
