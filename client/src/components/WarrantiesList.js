...
const WarrantiesList = () => {
		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getWarranties/`,
				);
	...
	if (sortOption === "idate-asc") {
		...
		sortedWarranties.sort(
			(a, b) => new Date(b.oservice_date) - new Date(a.service_date),
		);
	...
	const filteredWarranties = sortedWarranties.filter(
		(warranty) =>
			(filterOption === "" ||
				warranty.inspection_completed.toString() === filterOption) &&
			(serviceFilterOption === "" ||
				warranty.service_completed.toString() === serviceFilterOption) &&
						...
						<Col lg="3">
							...
							>
								...
								<select
									...
									onChange={(e) => {
										setServiceFilterOption(e.target.value);
										setCurrentPage(1);
									}}
								>
											...
											<React.Fragment key={warranty.warranty_id}>
												<tr style={{ borderRadius: "20px", padding: "10px" }}>
													<td style={{ color: "#014c91" }}>
														<Link
															to={`/viewwarrantydetails/${warranty.warranty_id}`}
															style={{ color: "#014c91" }}
														>
															#{String(warranty.warranty_id).padStart(4, "0")}
														</Link>
													</td>
													<td style={{ color: "#014c91" }}>
														#{String(warranty.sales_id).padStart(4, "0")}
													</td>
													...
													<td
														style={{
															color:
																warranty.inspection_date !== null &&
																warranty.inspection_completed
																	? "#008000"
																	: warranty.inspection_date !== null &&
																		  !warranty.inspection_completed
																		? "#DC6601"
													...
													>
														...
														{warranty.inspection_date !== null
															? `${formatDate(warranty.inspection_date)} ${new Date(warranty.inspection_date).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`
															: "-"}
													</td>
													<td
														style={{
															color: warranty.service_completed ? "#008000" : "#DC6601",
														}}
													>
														{warranty.service_completed ? (
															<FaCheck size={20} />
														) : (
															<FaClock size={20} />
														)}
														{formatDate(warranty.service_date)}{" "}
														{new Date(warranty.service_date).toLocaleTimeString([], {
															hour: "2-digit",
															minute: "2-digit",
														})}
													</td>
