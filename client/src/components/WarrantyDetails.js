		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getWarranty/${id}/`,
				);
				setWarrantyData(response.data[0]);
				console.log(response.data[0]);

				const response2 = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getClaimedUnits/${id}/`,
				);
				setClaimedUnitsData(response2.data);

				const response3 = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getWarrantyRequestedParts/${id}/`,
				);
					...
					<Row>
						<Col lg="3">
							...
							>
								{warrantyData.inspection_date !== null ? (
									<Card
										style={{
											padding: "15px",
											borderRadius: "20px",
											background: warrantyData.inspection_completed ? "#E7FFE2" : "white",
											color: warrantyData.inspection_completed ? "#008000" : "#014c91",
										}}
									>
										<Row>
											<Col lg="9">
											...
											<Col className="d-flex justify-content-end">
												<EditWarrantyServiceModal
													service_id={warrantyData.inspection_id}
													id={id}
													type={"inspection"}
													is_completed={warrantyData.inspection_completed}
													date={warrantyData.inspection_date}
												/>
											</Col>
										</Row>
										{!warrantyData.inspection_completed ? (
											<>
												...
												<Row>
													<Col>
														...
														<strong>
															{" "}
															{new Date(warrantyData.inspection_date).toLocaleTimeString([], {
																hour: "2-digit",
																minute: "2-digit",
															})}
														</strong>
												...
												<Row>
													<Col>
														Technician:{" "}
														<strong> {warrantyData.inspection_technician_name}</strong>
													</Col>
												</Row>
											</>
										) : (
								...
								<Card
									className={warrantyData.inspection_date !== null ? "mt-2" : null}
									style={{
										padding: "15px",
										borderRadius: "20px",
										height: "100%",
										background: warrantyData.service_completed ? "#E7FFE2" : "white",
										color: warrantyData.service_completed ? "#008000" : "#014c91",
									}}
								>
									<Row>
										<Col lg="9">
										...
										<Col className="d-flex justify-content-end">
											<EditWarrantyServiceModal
												service_id={warrantyData.warranty_service_id}
												id={id}
												type={"service"}
												is_completed={warrantyData.service_completed}
												date={warrantyData.service_date}
											/>
										</Col>
									</Row>
									{!warrantyData.service_completed ? (
										<>
											...
											<Row>
												<Col>
													...
													<strong>
														{" "}
														{new Date(warrantyData.service_date).toLocaleTimeString([], {
															hour: "2-digit",
															minute: "2-digit",
														})}
													</strong>
											...
											<Row>
												<Col>
													Technician:{" "}
													<strong> {warrantyData.service_technician_name}</strong>
												</Col>
											</Row>
										</>
									) : (
														...
														<tr style={{ borderRadius: "20px", padding: "10px" }}>
															...
															<td style={{ color: "#014c91" }}>
																{part.date_claimed === null ? (
																	<ClaimWarrantyPartModal
																		description={part.description}
																		quantity={part.quantity}
																		id={part.requested_part_id}
																	/>
																) : (
																	"Claimed"
																)}
