...
const useAddressFilter = (formData, setFormData) => {
	...
	const fetchData = async () => {
		try {
			const locFormsResponse = await axios.get(
				`${process.env.REACT_APP_ROOT_URL}/api/getLocationsForAddressInForms/`,
			);
	...
	useEffect(() => {
		const filterProvince = () => {
			const filteredProvince = province.filter(
				(prov) => prov.region_id === Number(formData.addr_region_id),
			);
		...
		const filterCity = () => {
			const filteredCity = city.filter(
				(cit) => cit.province_id === Number(formData.addr_province_id),
			);
		...
		const filterBarangay = () => {
			const filteredBarangay = barangay.filter(
				(bar) => bar.municipality_id === Number(formData.addr_municipality_id),
			);
