		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_ROOT_URL}/api/getTechnicians/`,
				);
				...
				try {
					const promises = allTechnicians.map(async (technician) => {
						const valid = await axios.get(
							`${process.env.REACT_APP_ROOT_URL}/api/getTechnicianAvailability/${technician.technician_id}`,
					...
					const availTechniciansData = allTechnicians.filter(
						(technician, index) => results[index],
					);
