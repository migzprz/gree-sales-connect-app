...
export default function useProducts(setLoading1) {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const productsResponse = await fetchDataAndMap(
					`${process.env.REACT_APP_ROOT_URL}/api/getProducts/`,
					"product",
				);
				const servicesResponse = await fetchDataAndMap(
					`${process.env.REACT_APP_ROOT_URL}/api/getServices/`,
					"service",
				);
				const partsResponse = await fetchDataAndMap(
